...
class FileDialogdemo(QWidget):
    def __init__(self, parent=None):
        ...
        # 这个是为了避免用户不读取目标行人就直接开始识别的bug而特意初始化的。在识别视频的部分会对其进行检测，如若为None，则视频为普通监控视频
    ...
    def playvideo(self):
        while self.running & self.cap.isOpened():
            ...
            # 需要在此处添加上对图片进行处理的部分！
            # 大致样子就是：imgAfterDetect=function(frame,self.originalImage)
            # 然后把后面的改正到这里
